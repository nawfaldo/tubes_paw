...
<head>
    ...
    <title>Edit Lomba - SIPRESMATU Admin</title>
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.lomba.update', $lomba->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3" required>{{ old('deskripsi', $lomba->deskripsi) }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label for="deadline_pendaftaran" class="form-label">Deadline Pendaftaran</label>
                                <input type="date" class="form-control" id="deadline_pendaftaran" name="deadline_pendaftaran" value="{{ old('deadline_pendaftaran', $lomba->deadline_pendaftaran) }}" required>
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="tingkat_lomba" name="tingkat_lomba" required>
                                    ...
                                    @foreach($tingkatOptions as $key => $label)
                                        <option value="{{ $key }}" {{ old('tingkat_lomba', $lomba->tingkat_lomba) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                @if($lomba->poster)
                                    <div class="mt-2">
                                        <img src="{{ asset('storage/' . $lomba->poster) }}" alt="Poster Lomba" class="img-fluid rounded" style="max-width: 200px;">
                                        <p class="text-muted mb-0">Poster saat ini</p>
                                    </div>
                                @endif
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    <option value="aktif" {{ old('status', $lomba->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                    <option value="selesai" {{ old('status', $lomba->status) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                                    <option value="dibatalkan" {{ old('status', $lomba->status) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="max_anggota" class="form-label">Maksimal Anggota Tim</label>
                                <input type="number" class="form-control" id="max_anggota" name="max_anggota" min="0" max="20" value="{{ old('max_anggota', $lomba->max_anggota) }}" required>
                            </div>
